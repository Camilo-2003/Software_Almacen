...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->execute()) {
        echo "<script>alert('✅Registro de instructor exitoso.'); window.location.href='../GestionUsuarios.php';</script>";
    } else {
        echo "<script>alert('⚠️Error al registrar instructor. ¡Instructor ya registrado!.'); window.location.href='../GestionUsuarios.php';</script>";
    }
    $stmt->close();
    $conexion->close();
    exit();

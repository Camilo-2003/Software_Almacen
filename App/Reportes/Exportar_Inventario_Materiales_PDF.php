...
<body>
    <h1>Reporte de Inventario de Materiales | SENA</h1>
...
if (!empty($materiales_data)) {
    ...
    <table>
        ...
        <tbody>';
    foreach ($materiales_data as $row) {
        $html .= '
            <tr>
                ...
                <td>' . htmlspecialchars($row['estado_material']) . '</td>
            </tr>';
    }
    $html .= '
        </tbody>
    ...
    $html .= '<p class="no-records">No hay materiales en el inventario para mostrar.</p>';
}
...
$filename_pdf = 'Inventario_Materiales_' . date('Ymd_His') . '.pdf';

...
<head>
    <meta charset="UTF-8">
    <title>Observaciones de Devolución</title>
    <style>
        ...
        th, td { border: 1px solid #ddd; padding: 5px; text-align: left; vertical-align: top;} /* Ajuste de alineación vertical */
...
<body>
    <h1>Reporte de Observaciones de Devolución | SENA</h1>
    <p class="generation-date">Fecha de Generación: ' . date('d/m/Y h:i:s A') . '</p>
';

if (!empty($observaciones_data)) {
    $html .= '
    <table>
        <thead>
            <tr>
        ...
        <tbody>';
    foreach ($observaciones_data as $row) {
        ...
        $fecha_devolucion_formateada = 'N/A';
        ...
        $observaciones_devolucion_display = empty($row['observaciones_devolucion']) ? 'Sin observaciones.' : htmlspecialchars($row['observaciones_devolucion']);
    ...
    $html .= '<p class="no-records">No hay registros de observaciones de devolución para mostrar.</p>';
}
...
$filename_pdf = 'Observaciones_Materiales_' . date('Ymd_His') . '.pdf';

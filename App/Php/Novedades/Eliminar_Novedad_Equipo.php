...
if (!$id_novedad) {
    header("Location: ../../Novedades_Equipos.php?mensaje=error_id");
    exit();
}
...
try {
    // Eliminar la novedad
    $sql_delete = "DELETE FROM novedades2 WHERE id_novedad2 = ? AND tipo_elemento = 'equipo'";
    ...
    $stmt->close();
    $conexion->commit();
    header("Location: ../../Novedades_Equipos.php?mensaje=eliminada");
    exit();

} catch (Exception $e) {
    $conexion->rollback();
    error_log("Error al eliminar novedad de material: " . $e->getMessage());
    header("Location: ../../Novedades_Equipos.php?mensaje=error2"); 
    exit();
} finally {

...
$ruta_imagen_existente = filter_input(INPUT_POST, 'imagen_existente', FILTER_SANITIZE_STRING);
...
if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
    ...
    if (!is_dir($directorio_destino)) {
        mkdir($directorio_destino, 0755, true); // Crear el directorio si no existe
    }
...
try {
    ...
    $sql_update = "UPDATE novedades2 SET 
                    ...
                    tipo_novedad = ?,
                    ruta_imagen = ?
                  WHERE id_novedad2 = ? AND tipo_elemento = 'material'";
    ...
    $stmt->close();
    $conexion->commit();
    header("Location: ../../Novedades_Materiales.php?mensaje=actualizada");
    exit();

} catch (Exception $e) {
    $conexion->rollback();
    error_log("Error al actualizar novedad de material: " . $e->getMessage());
    header("Location: ../../Novedades_Materiales.php?mensaje=error_actualizacion");
    exit();
} finally {

...
<div class="container historial-container">
    <div class="table-wrapper">
        <table id="tablaNovedades">
            ...
            <tbody>
                <?php
                $novedades_equipos = $conexion->query("SELECT * FROM novedades2 WHERE tipo_elemento = 'equipo' ORDER BY fecha_novedad DESC");  
                    if ($novedades_equipos->num_rows > 0) {
                    while ($datos = $novedades_equipos->fetch_assoc()) {
                        echo "<tr>
                                    ...
                                    <td class='obs'>" . htmlspecialchars($datos['tipo_novedad']) . "</td>
                                     <td>";
                                    ...
                                    <td class='acciones'>
                                        ...
                                        <button class='btn-eliminar' data-id='" . htmlspecialchars($datos['id_novedad2']) . "' data-delete-url='Php/Novedades/Eliminar_Novedad_Equipo.php?id=" . htmlspecialchars($datos['id_novedad2']) . "'><i class='fas fa-trash-alt' id='ii'></i> Eliminar</button>    
                                    </td>                          
                                    </tr>";
                    }
                } else {
...
<div id="modalEditarNovedad" class="modal" style="display: none;">
    <div class="modal-content">
        ...
        <h2>Editar Novedad de Equipo</h2>
        <form id="formEditarNovedadMaterial" method="POST" action="/Software_Almacen/App/Php/Novedades/Actualizar_Novedad_Equipo.php" enctype="multipart/form-data">
            <input type="hidden" id="edit_id_novedad" name="id_novedad2">
                ...
                <input type="text" id="edit_rol_responsable" name="rol_responsable_registro" required readonly>

                <label for="edit_nombre_responsable">Nombre del Responsable:</label>
                <input type="text" id="edit_nombre_responsable" name="nombre_responsable_registro" required readonly>

                <label for="edit_nombre_instructor">Instructor:</label>
                <input type="text" id="edit_nombre_instructor" name="nombre_instructor" required readonly>
